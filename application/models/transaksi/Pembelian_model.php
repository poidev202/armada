...
class Pembelian_model extends MY_Model {
	...
	{
		parent::__construct("transaksi_pembelian");
	}
	...
	public function transactionInsert($dataLaporanGudang,$data,$dataLpKas)
	{
		...
		$dataLpKas["pembelian_id"] = $pembelian_id;
	...
	public function transactionUpdate($data,$dataLaporanGudang)
	{
		...
		/*update for transaksi pembelian */
		$this->db->where("id",$data["id"]);
		$this->db->update("transaksi_pembelian",$data);
		/*end update for transaksi pembelian */
	...
	public function transactionDelete($id,$id_laporan)
	{
		...
		$this->db->delete("transaksi_pembelian",array("id" => $id));
	...
	public function findDataPemblian($where=false,$select=false,$orderBy=false,$search=false,$join=false,$limit=0,$offset=0)
	{
	...
	public function getCountPembelian($where=false,$select=false,$search=false,$join=false)
	{
		$this->db->from($this->_table);
		if($where) {
			if ($where["data"] != null) {
	...
	public function findDataTablePembelian($where=false,$select=false,$columnsOrderBy=false,$search=false,$join=false)
	{
		$input = $this->input;
		$orderBy = false;
		...
		$data = $this->findDataPemblian($where,$select,$orderBy,$search,$join,$input->post("length"),$input->post("start"));
	...
	public function findDataTableOutputPembelian($data=null,$where=false,$select=false,$search=false,$join=false)
	{
		$input = $this->input;
		if ($search) {
			$dataSearch = array();
		...
		$response->recordsTotal = $this->getCountPembelian($where,$select,$search,$join);
		$response->recordsFiltered = $this->getCountPembelian($where,$select,$search,$join);
		$response->data = $data;
...
/* Location: ./application/models/transaksi/Pembelian_model.php */